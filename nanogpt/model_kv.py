...
class CausalSelfAttention(nn.Module):
    ...
    # CHANGE: past_kv argument added
    def forward(self, x, past_kv=None):
        """
        EDIT 1: added kv caching here, and i will comment on where i did the changes with CHANGE
        """
        B, T, C = (
            x.size()
        ...
        if self.flash:
            # CHANGE: only turn on causal if there is no past_kv. No longer need causal masking for the entire sequence
...
class Block(nn.Module):
    """
    EDIT 2: modified to accept and return past_kv
    """
    ...
    # CHANGE: accepts and returns past_kv
    def forward(self, x, past_kv=None):
        # self-attention returns both the output and updated kv cache
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, past_kv=None):
        """
        EDIT 3: also added past_kv and logic to handle kv caching
        """
        device = idx.device
        b, t = idx.size()
        assert (
        ...
        if past_kv is not None and past_kv[0] is not None:
            past_length = past_kv[0][0].size(
                2
            )  # get the length of the past sequence (past_seq_len) as shown here (B, nh, past_seq_len, hs)
